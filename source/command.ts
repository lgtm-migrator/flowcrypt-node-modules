...
export const new_command = (cmd: string, allow_options: string[], env_config: BaseConfig, cb: (context: Context, local_opt: CommandLineOptions, ...args: string[]) => void) => {
  return (all_opt: CommandLineOptions, ...args: string[]) => {
    ...
    for(let opt_name of Object.keys(all_opt)) {
      if(env_config.is_configurable(opt_name)) {
        ...
        new Log(env_config).fatal(`Unknown option for ${cmd}: ${opt_name}. Allowed options: ${allow_options}\nAllowed global options: ${env_config.list_configurable(true)}`);
      }
    }
